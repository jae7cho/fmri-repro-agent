...
The committed example (``examples/spec.json``) is a CURRENT (0.5.1) document and is
validated with the current root. A coherent v0.1.0 document is no longer constructible
(nested ``Preprocessing.schema_version`` is ``Literal["0.5.1"]``), so version pinning is
...
def test_current_studyspec_accepts_matching_nested_stamp() -> None:
    # The example carries the current preprocessing fields; labeling it 0.5.1 and validating
    # under the current root passes the cross-validator (top version == nested stamp, both
    # defaulting to 0.5.1).
    payload = _example_payload()
    payload["schema_version"] = "0.5.1"
    study = CurrentStudySpec.model_validate(payload)
    assert study.schema_version == "0.5.1"
    for spec in study.specs:
...
def test_current_studyspec_rejects_mismatched_nested_stamp() -> None:
    ...
    # the first line of defense is Preprocessing.schema_version being a Literal["0.5.1"] (so a
...
def test_export_schema_writes_expected_file(
    ...
    # schema_version is pinned to "0.5.1" (Literal exports as const or enum).
    # Concrete literal (NOT derived) so an accidental future bump fails this test.
    sv = props["schema_version"]
    assert sv.get("const") == "0.5.1" or sv.get("enum") == ["0.5.1"]
